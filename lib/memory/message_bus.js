...
MessageBus.prototype.send = function (from, to, message, cb) {
    ...
    if (self.peers[to] === null || self.peers[to] === undefined) {
        return (setImmediate(cb.bind(
            null, new error.InternalError(
                sprintf('peer %s isn\'t known', to)))));
    }
    //Eat the message if either the to/from are partitioned.  Open question
    // whether it should return an error after so many ticks if not cancelled.
