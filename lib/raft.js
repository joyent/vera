...
function Raft(opts) {
    ...
    self.requestVotes = [];
    self.requestVoteInProgress = false;
...
/**
 * These need to be strictly serialized, so this simply appends to a queue.
 * They are processed in the order that they arrived in...
 */
Raft.prototype.requestVote = function (req, cb) {
    ...
    var self = this;
    var log = self.log;
    log.debug({ 'requestVote': req, 'queueLength': self.requestVotes.length },
              'appending to request votes queue');
    self.requestVotes.push({ 'req': req, 'cb': cb });
    self._requestVote();
};


/**
 * Internal API to process request votes serially.
 */
Raft.prototype._requestVote = function () {
    var self = this;
    var log = self.log;

    if (self.requestVotes.length === 0 || self.requestVoteInProgress) {
        return;
    }

    var rv = self.requestVotes.shift();
    var req = rv.req;
    var cb = rv.cb;
    self.requestVoteInProgress = true;
...
Raft.prototype.appendEntries = function (req, cb) {
    ...
    function appendToCLog(_, subcb) {
        ...
        self.clog.append(req.entries, subcb);
    }
