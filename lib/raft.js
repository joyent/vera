...
function _updateClusterConfig(req, cb) {
    ...
    console.log(self.clog.clusterConfig);
    process.exit(1);
    setImmediate(cb);
}
...
function _handleRaftRequest(command, cb) {
    ...
    //Now send it through clientRequest (yes, there's a feedback loop here)
//    console.log(self.cluster, 'cluster before');
    self.clientRequest({ 'command': changeCommand }, function (err) {
//        console.log(self.cluster, 'cluster after');
    });
}
