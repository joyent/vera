...
//This handles cluster reconfiguration on the client.
function _updateClusterConfig(req, cb) {
    ...
    self.updateCluster(self.clog.clusterConfig);
    setImmediate(cb);
}
...
function _handleRaftRequest(command, cb) {
    ...
    self.clientRequest({ 'command': changeCommand }, function (err) {
        ...
        self.clientRequest({ 'command': changeCommand }, function (err2) {
            ...
            //Leader steps down if not part of the cluster anymore
            if (self.state === STATES.leader &&
                !self.cluster.exists(self.id)) {
                self.log.debug({
                    'cluster': self.cluster
...
Raft.prototype.updateCluster = function (clusterConfig) {
