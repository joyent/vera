 ...
 * to look for 'noop'.
 */
function NoopEntriesTransform(a) {
...
Raft.prototype.installSnapshot = function (req, cb) {
    ...
    //TODO: Make a pipeline so that we don't try to do 2 at the same time.

    self.snapshotter.read(self, req.snapshot, function (err, snapshot) {
        if (err) {
            return (cb(err));
        }
        ...
        self.filteredPeers = [];
        snapshot.peers.forEach(function (p) {
            if (p !== self.id) {
                self.filteredPeers.push(p);
            }
        });
 ...
 * machine and is destined for raft-specific commands.  These are all valid
