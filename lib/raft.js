...
Raft.prototype.requestVote = function (req, cb) {
    ...
    function verifyTermAndStatus(_, subcb) {
        ...
        if (req.term > currentTerm) {
            ...
            //TODO: It's not very efficient to do an fsync here, then again
            // later if it turns out we're going to vote for this candidate.
            // We should really combine all these functions together...
    ...
    //TODO: Should be fairly simple to get a race condition here where two
    // candidates get to the same voteGranted.  We should be serializing these
    // requests...
    function castVote(_, subcb) {
        var votedFor = self.properties.get(VOTED_FOR);
        ...
        if (voteGranted) {
            self.updateVotedFor(req.candidateId, function () {
