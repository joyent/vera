...
Raft.prototype.sendAppendEntries = function (index, cb) {
    ...
    function onResponse(err, from, res) {
        ...
        if (err && err.name === 'NotBootstrappedError') {
            self.sendInstallSnapshot(from, function (err, res) {
                if (err) {
                    self.log.error({
                        'error': err,
                        'to': from
                ...
                self.peerNextIndexes[from] = res.index + 1;
                self.peerMatchIndexes[from] = res.index;
...
Raft.prototype.sendInstallSnapshot = function (to, cb) {
    ...
    if (self.peersSnapshotting[to] !== undefined) {
        ...
        self.snapshotter.getLatest(function (err, snapshot) {
            ...
            self.send(to, req, function (err, res) {
