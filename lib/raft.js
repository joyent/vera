...
function _verifyTermAndStatus(req, cb) {
    ...
    function checkParams(_, subcb) {
        ...
        //TODO: This is going to need to change when entries becomes a stream.
        var lastEntryIndex = req.entries[req.entries.length - 1].index;
        if (req.commitIndex > lastEntryIndex) {
            return (subcb(new error.InvalidIndexError(sprintf(
                'request commit index %d is ahead of the last log entry ' +
                    'index %d', req.commitIndex, lastEntryIndex))));
        }
        return (subcb());
    }
