...
function _getLatestEntries(req, cb) {
    ...
    // last index.
    self.clog.slice(smIndex + 1, req.commitIndex + 1, function (err, entries) {
        if (err) {
            log.error({ 'err': err,
                        'smIndex': smIndex,
...
Raft.prototype.clientRequest = function (req, cb) {
    ...
    function appendToCLog(_, subcb) {
        //This is choked down at the clog, right?
    ...
    //TODO: Need to correctly set the commit index.
    ...
    function executeEntry(_, subcb) {
        //This could use the same mechanism as append entries (above)
