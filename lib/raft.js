...
function Raft(opts) {
    ...
    self.clog = opts.clog;
    self.filteredPeers = filterPeers(self.id, self.clog.clusterConfig);
    self.majority = Math.floor(self.filteredPeers.length / 2) + 1;
...
/**
 * Combs entries out of streams that need to be handled by raft.  Specifically,
 * cluster configuration changes.
 *
 * This isn't actually transforming anything.
 */
function EntryComb() {
    this.entries = [];
    stream.Transform.call(this, { 'objectMode': true });
}
util.inherits(EntryComb, stream.Transform);

EntryComb.prototype._transform = function (entry, encoding, cb) {
    var self = this;
    if ((typeof (entry)) === 'object') {
        if (entry.to !== undefined && entry.to === 'raft' &&
            entry.command !== undefined &&
            entry.command.execute === 'clusterConfigure') {
...
EntryComb.prototype.getEntries = function () {
    return (this.entries);
};
...
function _requestVote(req, cb) {
    ...
    if (self.filteredPeers.indexOf(req.candidateId) === -1) {
        log.error({ 'candidateId': req.candidateId,
                    'peers': self.filteredPeers },
...
function _verifyTermAndStatus(req, cb) {
    ...
    function checkParams(_, subcb) {
        //I don't know who you are....
        if (self.filteredPeers.indexOf(req.leaderId) === -1) {
...
function _appendToCLog(req, cb) {
    ...
    var self = this;
    //TODO: Remove the entry comb from here... we need it in the log.
    var comb = new EntryComb();
    req.entries.pipe(comb);
    ...
    self.clog.append({
        ...
        cb(null, { req: req, comb: comb });
    });
}


function _updateClusterConfig(opts, cb) {
    assert.arrayOfObject(opts, 'opts');
    assert.ok(opts.length === 1);
    opts = opts[0];
    assert.object(opts.req, 'opts.req');
    assert.object(opts.comb, 'opts.comb');

    //var self = this;
    var req = opts.req;
    var entries = opts.comb.getEntries();
    if (entries.length === 0) {
        setImmediate(cb.bind(null, null, req));
        return;
    }
...
function _installSnapshot(req, cb) {
    ...
    self.snapshotter.read(req.snapshot, function (err, snapshot) {
        ...
        //TODO: Factor out into some sort of init?
 ...
 * object.)
 */
function filterPeers(id, clusterConfig) {
    assert.string(id, 'id');
    ...
    if (clusterConfig.current) {
        Object.keys(clusterConfig.current).forEach(function (p) {
            if (p !== id) {
                filteredPeers.push(p);
            }
        });
    }
 ...
 * # manually promoted.  TODO: Implement
 * { "to": "raft", "execute": "addPeer", "name": "[name]",
 *   "connection": ..., "autoPromote": [bool] }
 ...
 * # promoted to be a leader.  TODO: Implement
 * { "to": "raft", "execute": "promotePeer", "name": "[name]" }
 * # Removes a peer, regardless of status.  TODO: Implement
 * { "to": "raft", "execute": "removePeer": "name" }
 *
 * --- Manually mess with this raft instance ---
 * # Force a step down, only if it is a leader.  TODO: Implement
...
function _handleRaftRequest(command, cb) {
    ...
    //TODO:
    // 1) Reject a cluster reconfig request if we're already in the middle of
    //    another one (don't accept more until Cnew is *committed*.
    // 2) Return idempotently if the change has already been made (or is in
    //    progress).
    ...
    case 'stepDown':
        ...
        break;
    default:
        setImmediate(cb.bind(null, new error.InvalidRaftRequestError(
...
Raft.prototype.sendAppendEntries = function (index, cb) {
    ...
    if (self.filteredPeers.length < 1) {
    ...
    self.filteredPeers.forEach(function (peer) {
    ...
    vasync.pipeline({
        ...
        funcs: [
            ...
            function sendRequestVotes(_, subcb) {
                ...
                if (self.filteredPeers.length > 0) {
                    log.debug({
                        'peers': self.filteredPeers
...
Raft.prototype.summary = function () {
    ...
    return ({
        ...
        state: self.state,
        filteredPeers: self.filteredPeers,
        clog: {
