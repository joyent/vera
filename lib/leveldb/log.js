    ...
    function setupSelf() {
        ...
        opts.db.get(logKey(lastIndex), function (err, lastEntry) {
            ...
            opts.db.get(logKey(clusterConfigIndex), function (err2, config) {
                ...
                self.clusterConfig.clogIndex = clusterConfigIndex;
                self.db = opts.db;
                self.open = true;
            ...
            while (null !== (pair = ps.read())) {
                ...
                if (l !== null && l !== undefined && !truncated) {
                    ...
                    if (e.term !== l.term) {
                        ...
                        //TODO: Can we safely walk the leveldb for the last
                        // cluster configuration at this point?  When?
                        truncated = true;
                ...
                if (l === undefined || l === null || truncated) {
                    ...
                    //TODO: Check for cluster reconfigurations...
                    db.batch([
                        { 'type': 'put',
                          'key': logKey(e.index),
                          'value': e
