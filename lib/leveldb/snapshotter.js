...
Snapshotter.prototype.getLatest = function (cb) {
    ...
    var jtrans = new JsonTransform();
    rs.pipe(jtrans);

    setImmediate(cb.bind(null, null, jtrans));
};
...
Snapshotter.prototype.read = function (snapshot, cb) {
    ...
    //TODO: Catch possible errors thrown from here.
    snapshot.pipe(new lstream()).pipe(new ObjectTransform()).pipe(ws);
};
